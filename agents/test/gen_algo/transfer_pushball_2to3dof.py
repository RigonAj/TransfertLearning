...
class PushBallTransfer2to3:
    def __init__(self, policy_2dof_path: str, vecnorm_2dof_path: str,
        ...
        # Le checkpoint attend une entrée 40D (8 spatial + 4×8 historique)
        self.state_mapper = StateMapperMLP(40, 6).to(device)
        self.state_mapper.load_state_dict(checkpoint["state_mapper"])
        self.state_mapper.eval()

        # action_mapper attend state_dim + act_in_dim (ici 40 + 4)
        self.action_mapper = ActionMapperMLP(40, 4, 3).to(device)
    ...
    def _build_history_features(self, arm_state: np.ndarray) -> np.ndarray:
        ...
        historique glissant de 4 états : spatial (8) + 4×8 historique = 40 dims.
        """
    ...
    def predict(self, obs_3dof: np.ndarray) -> np.ndarray:
        ...
        # Le mapper attend 40 dims (8 spatial + 4×8 historique)
