...
class PushBallEnv_3dof(Arm3DoF):
    ...
    def step(self, action):
        ...
        new_theta2 = np.clip(
            ...
            self.theta_min, self.theta_max)
        new_theta3 = np.clip(
            self.theta3 + float(action[2]) * self.omega_max * self.dt,
            self.theta_min, self.theta_max)
...
##        if progress_ball_target > 0:
    ...
    def render(self):
        ...
        j3  = np.array([
            self.l1 * np.cos(self.theta1) + self.l2 * np.cos(self.theta1 + self.theta2),
            self.l1 * np.sin(self.theta1) + self.l2 * np.sin(self.theta1 + self.theta2),
        ])
        eff = self.forward_kinematics(self.theta1, self.theta2, self.theta3)

        plt.plot([0, j2[0]], [0, j2[1]], 'r-', lw=4, label='Link 1')
        plt.plot([j2[0], j3[0]], [j2[1], j3[1]], 'b-', lw=4, label='Link 2')
        plt.plot([j3[0], eff[0]], [j3[1], eff[1]], 'm-', lw=4, label='Link 3')
