...
from dataset import TrajectoriesTrainingDataset
from torch.utils.data import DataLoader, random_split
from mapper_models import StateMapperMLP
...
PATH_R1_TRAJ = "trajectories/{}.txt".format(R1_NAME)
PATH_R2_TRAJ = "trajectories/{}.txt".format(R2_NAME)
...
full_dataset = TrajectoriesTrainingDataset(
    PATH_R1_TRAJ, PATH_R2_TRAJ, STATE_DIM_R1, STATE_DIM_R2
)
...
def evaluate_val(loader):
    ...
    sum_r2_to_r1 = 0.0
    n = 0
    for state_r1, state_r2 in loader:
        sum_r1_to_r2 += mse_loss(model_r1_to_r2(state_r1), state_r2).item()
        sum_r2_to_r1 += mse_loss(model_r2_to_r1(state_r2), state_r1).item()
        n += 1
...
if __name__ == "__main__":
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        with torch.no_grad():
            ...
            s_r1, s_r2 = data_val[idx]

            pred_r2 = model_r1_to_r2(torch.from_numpy(s_r1).unsqueeze(0)).squeeze(0)
            pred_r1 = model_r2_to_r1(torch.from_numpy(s_r2).unsqueeze(0)).squeeze(0)

            print("  [r1→r2] target : {}".format([round(v, 4) for v in s_r2.tolist()]))
            print("          pred   : {}".format([round(v, 4) for v in pred_r2.tolist()]))
            print("  [r2→r1] target : {}".format([round(v, 4) for v in s_r1.tolist()]))
            print("          pred   : {}".format([round(v, 4) for v in pred_r1.tolist()]))
            print("  Pred std r1→r2 : {:.4f}  Target std: {:.4f}".format(
                pred_r2.std().item(), s_r2.std().item()))
    ...
    for name, model in [("r1_to_r2", model_r1_to_r2),
                         ("r2_to_r1", model_r2_to_r1)]:
