...
class TransferMapper:
    ...
    def __init__(
        ...
        self.in_arm_state_dim = (
            8 if in_state_dim in (8, 40)
            else 6
        )

        self.dof_in = (
            3 if self.in_arm_state_dim == 8
            else 2
        )
    ...
    def _state_loss(self, raw_pred_s, s_out_f, s_in_f):
        ...
        s_in_arm = s_in_f[..., :self.in_arm_state_dim]
        ref_eff = arm_effector_fields_torch(s_in_arm)
    ...
    def _fidelity(
        ...
        s_in_arm_batch = s_in_batch[..., :self.in_arm_state_dim]
        ee_real = arm_effector_fields_torch(s_in_arm_batch)
...
def main():
    ...
    def pad_last_dim(arr, target_dim):
    ...
    # If history variants exist, build combined inputs by concatenating
    # current spatial state + sliding state history along the last axis.
    if (
        "segments_2dof_history" in trajectories
        and "segments_3dof_history" in trajectories
    ):
        print("[INFO] History variants detected - creating combined inputs (spatial+history)")

        s2_hist = trajectories["segments_2dof_history"]
        s3_hist = trajectories["segments_3dof_history"]

        # basic shape checks
        if s2.shape[:2] != s2_hist.shape[:2] or s3.shape[:2] != s3_hist.shape[:2]:
            raise ValueError("Spatial and history state variants have mismatched segment/time shapes")
        if s2_hist.shape[-1] != 4 * s2.shape[-1] or s3_hist.shape[-1] != 4 * s3.shape[-1]:
            raise ValueError("History state variants must contain 4 concatenated states")

        # concatenate along feature dim
        s2_comb = np.concatenate([s2, s2_hist], axis=-1)
        s3_comb = np.concatenate([s3, s3_hist], axis=-1)

        # Use combined inputs as the mapper inputs; keep outputs original spatial
        s2_in = s2_comb
        s3_in = s3_comb
        a2_in = pad_last_dim(a2, 4)
        a3_in = a3

    else:
        print("[INFO] No history variants found - using spatial-only inputs")
        s2_in = s2
        s3_in = s3
        a2_in = pad_last_dim(a2, 4)
        a3_in = a3
