"""
replay_pushball_3dof.py
Rejoue les épisodes sauvegardés par runs_pushball_3dof.py.

Usage :
  python replay_pushball_3dof.py              # rejoue les succès (défaut)
  python replay_pushball_3dof.py --success
  python replay_pushball_3dof.py --fail
  python replay_pushball_3dof.py --fail --ep 3
  python replay_pushball_3dof.py --success --delay 0.05
"""
...
# ── Arguments ─────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Replay push-ball 3-dof episodes")
...
data_dir = f"runs/pushball_3dof/{mode}"

# ── Chargement ────────────────────────────────────────────────────────────────
files = sorted(glob.glob(os.path.join(data_dir, "ep_*.npz")))
if not files:
    raise FileNotFoundError(
        f"Aucun fichier dans '{data_dir}'. Lancez runs_pushball_3dof.py d'abord."
    )
...
print(f"Replay [{mode.upper()}] — {len(files)} épisode(s) — push-ball 3-DOF\n")
