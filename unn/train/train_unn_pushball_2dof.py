"""
Train UNN PPO policy for PushBall 2-DoF in Cartesian latent space (velocity-based).
...
# omega_max * max_reach = 2.0 * 3.0 = 6.0 m/s is the physical upper bound
# for this arm; using it means latent_action = 1.0 → full-speed EE motion.
V_MAX_EE = 6.0   # m/s
...
class UNNLatentEnv(gym.Wrapper):
    """
    Wraps PushBallEnv_2dof so PPO sees a 6D Cartesian latent space and
    ...
    def step(self, latent_action):
        ...
        link_lengths  = np.array([self.env.l1, self.env.l2], dtype=np.float32)
...
def main():
    ...
    def make_env(rank: int = 0):
        def _init():
            ...
            env = UNNLatentEnv(env, encoder, arm_obs_size=6, n_joints=2, v_max_ee=V_MAX_EE)
            env = Monitor(env)
            env.reset(seed=rank)
            return env
        return _init
    ...
    eval_callback = SyncEvalCallback(
        eval_env,
        best_model_save_path="./data/UNN/unn_pushball_2dof",
        log_path="./data/UNN/logs_2dof",
        eval_freq=10_000,
        n_eval_episodes=20,
        deterministic=True,
    )
    ...
    model = PPO(
        ...
        tensorboard_log="./data/UNN/logs_2dof",
        verbose=1,
    )
    ...
    link_lengths = [float(tmp_env.l1), float(tmp_env.l2)]
