"""
runs_transfer_pushball_3to2dof.py  (UNN)
Collecte N_TARGET épisodes réussis et N_TARGET échoués —
politique UNN entraînée sur 3-DOF transférée dans l'env 2-DOF.
Sauvegarde dans :
  runs/unn_transfer_pushball_3to2dof/success/ep_XXX.npz
  runs/unn_transfer_pushball_3to2dof/fail/ep_XXX.npz
"""
...
# ── Configuration ──────────────────────────────────────────────────────────────
POLICY_DIR  = "./data/UNN/unn_pushball_3dof"   # politique source : 3-DOF
POLICY_NAME = "final"
MAX_STEPS   = 150
N_TARGET    = 20
DEVICE      = "cpu"

SAVE_SUCCESS = "runs/unn_transfer_pushball_3to2dof/success"
SAVE_FAIL    = "runs/unn_transfer_pushball_3to2dof/fail"
os.makedirs(SAVE_SUCCESS, exist_ok=True)
os.makedirs(SAVE_FAIL,    exist_ok=True)

# ── Chargement politique UNN 3-DOF ────────────────────────────────────────────
...
print(f"Collecte de {N_TARGET} succès et {N_TARGET} échecs — UNN transfer 3→2 DOF\n")

while n_success < N_TARGET or n_fail < N_TARGET:
    obs       = env.reset()[0]         # (obs_dim,) — observation brute env 2-DOF
