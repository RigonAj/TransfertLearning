"""
replay_transfer_pushball_3to2dof.py  (UNN)
Rejoue les épisodes sauvegardés par unn/record/runs_transfer_pushball_3to2dof.py.
Politique UNN 3-DOF transférée, exécutée dans l'environnement 2-DOF.
...
# ── Arguments ─────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Replay UNN transfer pushball 3→2 DOF")
...
data_dir = f"runs/unn_transfer_pushball_3to2dof/{mode}"
...
if not files:
    raise FileNotFoundError(
        ...
        f"Lancez unn/record/runs_transfer_pushball_3to2dof.py d'abord."
    )
...
print(f"Replay [{mode.upper()}] — {len(files)} épisode(s) — UNN transfer 3→2 DOF\n")
...
for file_idx, fpath in enumerate(files):
    ...
    for step_i, (t1, t2) in enumerate(thetas):
        ...
        ax_robot.set_title(
            f"[UNN Transfer 3→2 DOF]  {ep_name}  [{mode}]  step {step_i+1}/{len(thetas)}\n"
