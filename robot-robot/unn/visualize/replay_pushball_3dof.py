"""
replay_pushball_3dof.py  (UNN)
Rejoue les épisodes sauvegardés par unn/record/runs_pushball_3dof.py.
...
# ── Arguments ─────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Replay UNN push-ball 3-dof episodes")
...
data_dir = f"runs/unn_pushball_3dof/{mode}"
...
if not files:
    raise FileNotFoundError(
        ...
        f"Lancez unn/record/runs_pushball_3dof.py d'abord."
    )
...
print(f"Replay [{mode.upper()}] — {len(files)} épisode(s) — UNN push-ball 3-DOF\n")
...
for file_idx, fpath in enumerate(files):
    ...
    for step_i, (t1, t2, t3) in enumerate(thetas):
        ...
        ax_robot.set_title(
            f"[UNN 3-DOF]  {ep_name}  [{mode}]  step {step_i+1}/{len(thetas)}\n"
