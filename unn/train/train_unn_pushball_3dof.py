"""
Train UNN PPO policy for PushBall 3-DoF in Cartesian latent space.
"""
...
class UNNLatentEnv(gym.Wrapper):
    """
    Wrap PushBallEnv_3dof to use Cartesian latent state (6D) and latent action (dx, dy).
    The action mapper converts (joint_angles, latent_action) into joint action.
    """
    def __init__(self, env, encoder, action_mapper, arm_obs_size, n_joints, max_reach=3.0):
        ...
        # Latent observation space: [eff_x, eff_y, ball_x, ball_y, tgt_x, tgt_y] normalized [-1,1]
        self.observation_space = spaces.Box(low=-1.0, high=1.0, shape=(6,), dtype=np.float32)
        # Latent action space: desired end‑effector displacement (dx, dy) normalized [-1,1]
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(2,), dtype=np.float32)

    @torch.no_grad()
    def step(self, latent_action):
        raw_obs = self.env._get_obs()
        # Extract joint angles (normalized by π) from raw observation
        joint_angles = raw_obs[:self.n_joints]   # θ1/π, θ2/π, θ3/π
        ...
        # Encode raw observation to latent state
...
def main():
    ...
    N_ENVS = 32
    TOTAL_TIMESTEPS = 50_000_000   # same as 2DoF

    # Load pre‑trained action mapper for 3DoF
    mapper_path = Path("./data/UNN/action_mappers/mapper_3dof.pt")
    if not mapper_path.exists():
        raise FileNotFoundError(f"Action mapper not found at {mapper_path}. Run train_action_mappers.py first.")
    mapper = ActionMapper(n_joints=3).to(DEVICE)
    ...
    def make_env(rank=0):
        def _init():
            ...
            env = UNNLatentEnv(env, encoder, mapper, arm_obs_size=8, n_joints=3)
            env = Monitor(env)
            env.reset(seed=rank)
            return env
        return _init
    ...
    class SyncEvalCallback(EvalCallback):
        def _on_step(self) -> bool:
            # Sync normalization stats between train and eval
    ...
    print("\nTraining UNN PPO on 3-DoF Cartesian latent space (6D)...")
    model.learn(total_timesteps=TOTAL_TIMESTEPS, callback=eval_callback, progress_bar=True)

    # Save final policy using UNNPolicy wrapper
    from unn.unn_policy import UNNPolicy
    unn_policy = UNNPolicy(encoder, model, train_env, mapper, arm_obs_size=8, n_joints=3, device=DEVICE)
