"""
replay_pushball_2dof.py  (UNN)
Rejoue les épisodes sauvegardés par unn/record/runs_pushball_2dof.py.
...
# ── Arguments ─────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Replay UNN push-ball 2-dof episodes")
...
data_dir = f"runs/unn_pushball_2dof/{mode}"
...
if not files:
    raise FileNotFoundError(
        ...
        f"Lancez unn/record/runs_pushball_2dof.py d'abord."
    )
...
print(f"Replay [{mode.upper()}] — {len(files)} épisode(s) — UNN push-ball 2-DOF\n")
...
for file_idx, fpath in enumerate(files):
    ...
    for step_i, (t1, t2) in enumerate(thetas):
        ...
        ax_robot.set_title(
            f"[UNN 2-DOF]  {ep_name}  [{mode}]  step {step_i+1}/{len(thetas)}\n"
