...
from agents.transfer_ppo.mapper_models import (
    StateMapperMLP,
    ActionMapperMLP,
...
class ReachingTransfer3to2:
    def __init__(self, policy_3dof_path: str, vecnorm_3dof_path: str,
        ...
        # Le checkpoint attendu : entrée 12D (6 spatial + 6 temporal)
        self.state_mapper = StateMapperMLP(12, 8).to(device)
        self.state_mapper.load_state_dict(checkpoint["state_mapper"])
        self.state_mapper.eval()

        # action_mapper attend state_dim + act_in_dim = 15 (ici 12 + 3)
        self.action_mapper = ActionMapperMLP(12, 3, 2).to(device)
    ...
    def _build_history_features(self, arm_state_2dof: np.ndarray) -> np.ndarray:
        """
        Extrait les features d'historique glissant à partir de l'état courant
        et les combine avec les features spatiales pour former l'entrée du mapper.
        Le mapper attend 12 dims (6 spatial + 6 temporal).
        """
        arm_2 = arm_state_2dof.copy()

        if len(self.history) == 0:
            self.history.append(arm_2.copy())

        self.history.append(arm_2.copy())

        hist = np.concatenate(list(self.history), axis=-1)

        return np.concatenate([arm_2, hist], axis=-1)
    ...
    def predict(self, obs_2dof: np.ndarray) -> np.ndarray:
        ...
        # Le mapper attend 30 dims (6 spatial + 4x6 history)
...
def main():
    ...
    MAPPER_PATH   = str(DATA_ROOT / "DIRECT_PPO" / "transfer_3to2_seq.pt")
