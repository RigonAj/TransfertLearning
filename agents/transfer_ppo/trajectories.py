...
def build_sliding_history(states, history_len=4):
    ...
    for i in range(n):
        ...
        for k in range(history_len):
            idx = max(0, i - history_len + 1 + k)
            seq.append(states[idx])

        out.append(np.concatenate(seq))
...
def record_one_segment(env2, env3, start_target, end_target,
    ...
    joint_vel_2 = (states2[:, 2:4] * env2.omega_max).astype(np.float32)
    joint_vel_3 = (states3[:, 3:6] * env3.omega_max).astype(np.float32)

    vel2 = states2[:, 2:4] * env2.omega_max
    vel3 = states3[:, 3:6] * env3.omega_max

    actions2 = vel2.astype(np.float32)
    actions3 = vel3.astype(np.float32)
...
def main(
    ...
    # joint-velocity segments (real dynamics)
    all_segments_vel2 = []
    all_segments_vel3 = []

    all_segments_s2_history = []
    all_segments_s3_history = []
    ...
    while valid_segments < N_SEGMENTS_TARGET:
        ...
        for i in range(len(targets) - 1):
            s2, s3, ee2, ee3, a2, a3, joint_vel_2, joint_vel_3, success, info = record_one_segment(
            ...
            # Spatial sampling for joint velocities (aligned by index, not EE)
            vel2_ss, _ = spatial_sampling_aligned(
                joint_vel_2,
                joint_vel_2,
                NUM_SAMPLES,
                use_effector=False,
            ...
            vel3_ss, _ = spatial_sampling_aligned(
                joint_vel_3,
                joint_vel_3,
                NUM_SAMPLES,
                use_effector=False,
            ...
            # Sliding state history over spatially sampled states.
            s2_hist = build_sliding_history(s2_ss, 4)
            s3_hist = build_sliding_history(s3_ss, 4)
            ...
            all_segments_vel2.append(vel2_ss)
            all_segments_vel3.append(vel3_ss)

            all_segments_s2_history.append(s2_hist)
            all_segments_s3_history.append(s3_hist)
    ...
    segments_s2_history = np.stack(all_segments_s2_history, axis=0)
    segments_s3_history = np.stack(all_segments_s3_history, axis=0)

    # Stack joint-velocity segments
    segments_vel2 = np.stack(all_segments_vel2, axis=0)
    segments_vel3 = np.stack(all_segments_vel3, axis=0)
    ...
    trajectories = {
        ...
        # Sliding state history variants
        'segments_2dof_history': segments_s2_history,
        'segments_3dof_history': segments_s3_history,

        # Joint-velocity segments (real dynamics)
        'segments_joint_velocity_2dof': segments_vel2,
        'segments_joint_velocity_3dof': segments_vel3,
