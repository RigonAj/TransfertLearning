"""
Collect paired trajectories for UNN.

Deprecated/optional: this script historically collected paired trajectories
from pretrained PPO policies to train learned action mappers. In the current
UNN pipeline we use analytic IK instead of learned mappers, and UNN is
trained directly via RL on the PushBall environment, so running this
collection is usually unnecessary.

Keep this script only if you need data for analysis or to reproduce older
experiments that relied on learned action mappers.
"""
...
def main():
    ...
    policy2, vec_norm2 = load_policy(MODEL_2, VEC_2, PushBallEnv_2dof)
    policy3, vec_norm3 = load_policy(MODEL_3, VEC_3, PushBallEnv_3dof)
