"""
runs_pushball_2dof.py
Collecte 20 épisodes réussis et 20 épisodes échoués pour le push-ball 2-DOF.
Sauvegarde les trajectoires complètes dans :
  runs/pushball_2dof/success/ep_XXX.npz
  runs/pushball_2dof/fail/ep_XXX.npz
"""
...
ROOT = next(parent for parent in Path(__file__).resolve().parents if (parent / "database" / "models").exists())
MODEL_ROOT = ROOT / "database" / "models"
...
SAVE_SUCCESS = "runs/ppo_pushball_2dof/success"
SAVE_FAIL    = "runs/ppo_pushball_2dof/fail"
...
print(f"Collecte de {N_TARGET} succès et {N_TARGET} échecs — push-ball 2-DOF\n")
...
while n_success < N_TARGET or n_fail < N_TARGET:
    ...
    while not done:
        ...
        thetas.append([infos[0]["theta1"], infos[0]["theta2"]])
        balls.append(infos[0]["ball"].copy())
