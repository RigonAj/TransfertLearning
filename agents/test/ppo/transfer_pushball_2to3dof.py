...
class PushBallTransfer2to3:
    ...
    def _build_history_features(self, arm_state_3dof: np.ndarray) -> np.ndarray:
        """
        ...
        """
        arm_3 = arm_state_3dof.copy()

        if len(self.history) == 0:
            self.history.append(arm_3.copy())

        self.history.append(arm_3.copy())

        hist = np.concatenate(list(self.history), axis=-1)

        return np.concatenate([arm_3, hist], axis=-1)
    ...
    def predict(self, obs_3dof: np.ndarray) -> np.ndarray:
        ...
        # Le mapper attend 40 dims (8 spatial + 4x8 history)
...
def main():
    ...
    MAPPER_PATH   = str(DATA_ROOT / "DIRECT_PPO" / "transfer_2to3_seq.pt")
